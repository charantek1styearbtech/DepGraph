...
export async function getProjectDetail(projectId, { maxDepth = 8 } = {}) {
  ...
  const vulnerabilityRecords = await runQuery(
    `MATCH (p:Project {id: $projectId})-[d:DIRECT_DEPENDS_ON]->(k:Package)-[:HAS_VERSION]->(v:Version)-[:AFFECTED_BY]->(c:Vulnerability)
     WHERE (d.resolvedVersion IS NULL AND v.isLatest) OR v.number = d.resolvedVersion
     RETURN DISTINCT c{.*, packageId: k.id, packageName: k.name, versionNumber: v.number}
  ...
  const vulnerabilities = vulnerabilityRecords.map((row) => row.c);
  return {
    ...baseRecords[0],
