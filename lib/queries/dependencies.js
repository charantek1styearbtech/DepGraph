 ...
 */
async function reverseDependencyDepths(
  packageId,
  { maxDepth = DEFAULT_MAX_DEPTH, maxNodes = 800 } = {},
) {
  const depths = new Map([[packageId, 0]]);
  let frontier = [packageId];
  let discovered = 1; // packageId itself
  ...
  for (let depth = 1; depth <= maxDepth && frontier.length > 0; depth += 1) {
    ...
    frontier = [...new Set(rows.map((row) => row.id))]
      .filter((id) => !depths.has(id))
      .slice(0, maxNodes - discovered); // hard cap prevents runaway BFS
